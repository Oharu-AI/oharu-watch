...
## 2026-08-26 国外記事のタイトル・要約翻訳を強化

- `AI最新情報（国外）` は短い英文も含め、タイトルと要約を優先して日本語化するよう判定を修正した。
- 保存済み記事の翻訳上限を40件から120件へ拡大し、Gemini由来の記事でも英語要約が残っていれば翻訳対象にした。
- 本文のない記事は、翻訳後の日本語タイトルを使った案内文を表示する。日本語文中に英語タイトルだけが残る問題を解消した。
- Googleの無料翻訳がHTTP 429になった場合、MyMemoryの無料翻訳へ自動で切り替える。翻訳リクエストには間隔を設け、両サービスのタイムアウト・接続切断でもニュース更新全体を停止しない。
- 新規記事にはRSSの元説明文を保存する。翻訳失敗で定型文になった国外記事は、次回RSS取得時に説明文を再取得できれば日本語要約へ自動修復する。
- GitHubの最新記事を取り込んだ保存データで国外127件を確認し、126件は日本語タイトル、残り1件は翻訳不要な製品型番のみ。要約は127件すべて日本語表示になった。
- `test_fetch_news.py` は翻訳・再修復・通信障害を含む12件が成功。Python構文検査も成功した。
