...
## 2026-08-23 A構成＋B配色をニュース本体へ正式反映

- ユーザー承認済みの `design-proposals/concept-ab-editorial-signal.html` を基準に、ニュース本体へ正式採用した。
- `generate_html.py` の一覧・記事詳細へ、黄色の速報帯、`OHARU/WATCH`ロゴ、ピル型カテゴリナビ、`LIVE / CURATED INTELLIGENCE`表示を追加した。今後の自動更新でもデザインは維持される。
- `style.css` を黒背景・白文字・黄色アクセントのニュースメディア調へ刷新した。
  - PC: 大きな注目記事＋短い注目一覧＋右カテゴリ欄、通常記事は3列。
  - iPhone: 1列表示、44px以上のナビ、16:9画像、横はみ出しなし。
- 見出しを「おすすめ」から「今日の注目」、「通常の記事」から「カテゴリ別ニュース」へ変更した。おすすめ5件と通常記事の重複排除処理は維持している。
- 実データ323件で `index.html` と `article.html` を再生成した。
- Python構文検査と既存テスト6件が成功。表示データはおすすめ5件、通常228件、両者の重複0件。
- ブラウザで1280×720と390×844を確認し、両方で横はみ出し0、画像崩れ0、コンソールエラー0。追加表示は国内AIで30件から50件へ増えることを確認した。
...
- 画面は黒背景・白文字・黄色アクセントを基調に、新聞型の情報配置、固定ヘッダー、スマホ用横スクロールナビを統一したニュースメディア調のデザイン。
